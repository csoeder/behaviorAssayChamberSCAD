...
module homefill(){
union(){
translate([-rail_width/2 + central_chamber_rad + chamber_spacing/2 + rail_width,-window_location+window_offset*-1,rail_height+ledge_height]) rotate([0,180,0]) slider(rail_height=rail_height,rail_width = 0.9*rail_width,rail_length = rail_length, synchole_rad=synchole_rad,window_length=window_length,window_height=window_height, window_location=window_location, tailhook_length=tailhook_length,tailhook_height=tailhook_height);
translate([rail_width - central_chamber_rad - chamber_spacing/2 - rail_width/2,-window_location+window_offset*1,rail_height+ledge_height]) rotate([0,180,0]) slider(rail_height=rail_height,rail_width = 0.9*rail_width,rail_length = rail_length, synchole_rad=synchole_rad,window_length=window_length,window_height=window_height, window_location=window_location, tailhook_length=tailhook_length,tailhook_height=tailhook_height);
    ...
    dish_wall_thk = 1;
    color([1,0,1,0.75])translate([0,0,dish_wall_thk +flyzone+ledge_height+bottom_bowl_depth_center])rotate([0,180,0])dish(outer_dia = central_chamber_rad*2,height=bottom_bowl_depth_center, wall_thk = dish_wall_thk);


    color([1,0,1,0.75])translate([0,0,-20+ledge_height-bottom_bowl_depth_center])rotate([0,0,0])dish(outer_dia = ledge_rad_center*2,height=bottom_bowl_depth_center, wall_thk = dish_wall_thk);



    color([1,0,1,0.75])translate([central_chamber_rad+side_chamber_rad+chamber_spacing,0,dish_wall_thk +flyzone+ledge_height+bottom_bowl_depth_side])rotate([0,180,0])dish(outer_dia = side_chamber_rad*2,height=bottom_bowl_depth_side, wall_thk = dish_wall_thk);

    color([1,0,1,0.75])translate([-(central_chamber_rad+side_chamber_rad+chamber_spacing),0,dish_wall_thk +flyzone+ledge_height+bottom_bowl_depth_side])rotate([0,180,0])dish(outer_dia = side_chamber_rad*2,height=bottom_bowl_depth_side, wall_thk = dish_wall_thk);

    color([1,0,1,0.75])translate([-(central_chamber_rad+ledge_rad_side+chamber_spacing),0,0+ledge_height-bottom_bowl_depth_center])rotate([0,0,0])dish(outer_dia = ledge_rad_side*2,height=bottom_bowl_depth_center, wall_thk = dish_wall_thk);
